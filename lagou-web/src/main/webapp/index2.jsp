...
<head>
  ...
  <title>JJA1912-拉勾网-最专业的互联网招聘平台</title>
  ...
  <div id="container">
    ...
    <div id="sidebar">
      <div class="mainNavs">
        <!--使用ajax动态渲染数据-->
        <!--一个一级类型的整个块-->
     <c:forEach items="${list}" var="t1" varStatus="st">
     <div class="menu_box">
          <div class="menu_main">
            <h2>${t1.onetypeName} <span></span></h2>
          </div>
          <div class="menu_sub dn" style="top:${st.index*57.6}px;border:1px solid red;">
          	<c:forEach items="${t1.list}" var="t2">
            <dl class="reset">
              <dt>
              ...
              <dd>
              	<c:forEach items="${t2.list}" var="t3">
              		<a href="h/jobs/list_Java?labelWords=label" class="curr">${t3.threetypeName}</a>
              	</c:forEach>
              </dd>
            </dl>
            </c:forEach>
    ...
    <script type="text/javascript" src="${pageContext.request.contextPath}/style/js/home.min.js"></script>
    <script type="text/javascript" src="${pageContext.request.contextPath}/style/js/count.js"></script>
    <div class="clear"></div>
  ...
  $(document).ready(function(){
	  ...
	  $.ajax({
		  ...
		  success:function(res){
			  var str = "";//最终拼接完的字符串
			  $.each(res,function(index,t1){
				  //一级循环一条记录
				  str = str + '<div class="menu_box"><div class="menu_main"><h2>'+t1.onetypeName+' <span></span></h2></div>';
			  	  str = str + '<div class="menu_sub dn" style="top:' + index*57.6 +'px;">';
				  //二级记录循环
			  	  $.each(t1.list,function(index,t2){
			  		  str = str + '<dl class="reset"><dt><a href="h/jobs/list_后端开发?labelWords=label">'+t2.twotypeName+'</a></dt>';
			  	  	  //三级记录循环
			  	  	  str = str + '<dd>';
			  	  	  $.each(t2.list,function(index,t3){
			  	  		  str = str + '<a href="h/jobs/list_Java?labelWords=label" class="curr">'+t3.threetypeName+'</a>';
			  	  	  });
			  	  	  str = str + '</dd>';
			  		  str = str + '</dl>';
			  	  })
      ...
      // 对动态元素绑定事件
      $(document).on("mouseover",".menu_box",function () {
          $(this).addClass("current");
          $(this).children(".menu_sub").removeClass("dn");
      })
      $(document).on("mouseout",".menu_box",function () {
          $(this).removeClass("current");
          $(this).children(".menu_sub").addClass("dn");
      })
  })
</script>
